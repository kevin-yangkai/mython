...
def draw(datasets='datasets/dataset.pkl'):
    np.random.seed(5)
    centers = [[1, 1], [-1, -1], [1, -1]]
    # iris = datasets.load_iris()
    iris = process_dataset_iris(datasets)
    X = iris.data
    y = iris.target
    ...
    for name, label in [('aerosmith', 0), ('beatles', 1),
                        ('creedence_clearwater_revival', 2), ('cure', 3), ('dave_matthews_band', 4),
                        ('depeche_mode', 5),
                        ('fleetwood_mac', 6), ('garth_brooks', 7), ('green_day', 8), ('led_zeppelin', 9),
                        ('madonna', 10),
