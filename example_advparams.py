...
class AdvRBM(base.RBM):
    def __init__(self, n_visible, n_hidden):
        ...
        # parameters
        self.W = parameters.AdvancedProdParameters(self, [self.v, self.h], [1, 1],
                                                   theano.shared(value=self._initial_W(), name='W'),
                                                   name='W')  # weights
        self.bv = parameters.AdvancedBiasParameters(self, self.v, 1, theano.shared(value=self._initial_bv(), name='bv'),
                                                    name='bv')  # visible bias
        self.bh = parameters.AdvancedBiasParameters(self, self.h, 1, theano.shared(value=self._initial_bh(), name='bh'),
                                                    name='bh')  # hidden bias
...
s = stats.cd_stats(rbm, initial_vmap, visible_units=[rbm.v], hidden_units=[rbm.h], k=1, mean_field_for_gibbs=[rbm.v],
...
evaluate = t.compile_function(initial_vmap, mb_size=100, monitors=[m, m_data, m_model, e_data, e_model],
                              name='evaluate', train=False, mode=mode)
...
for epoch in range(epochs):
    monitoring_data_train = [(cost, energy_data, energy_model) for cost, energy_data, energy_model in
                             train({rbm.v: train_set_x})]
    ...
    monitoring_data = [(cost, data, model, energy_data, energy_model) for cost, data, model, energy_data, energy_model
                       in evaluate({rbm.v: valid_set_x})]
