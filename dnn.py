...
class AutoEncoder(object):
    def __init__(self, supervised, unsupervised, targets, layers=[], hidden_layer="SigmoidLayer",
                 final_layer="SigmoidLayer", compression_epochs=100, verbose=True, bias=True,
                 autoencoding_only=True, dropout_on=True):
        """


        :type unsupervised: object
        :rtype : object
        """
        if len(layers) > 8:
        ...
        if hidden_layer == "SigmoidLayer":
            ...
            raise Exception("hidden_layer must be either: "
                            "'LinearLayer', 'SoftmaxLayer', 'SigmoidLayer', or 'TanhLayer'")
    ...
    def predict(self, data):
        if not self.nn:
            raise Exception("You must run ._train() before you can predict")
        for nn in self.nn:
    ...
    def dropout(self, data, noise=0.2, bag=1, debug=False):
        if bag < 1:
